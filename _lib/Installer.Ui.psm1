...
.SYNOPSIS
    ...
    ClusterIssuer, by deleting its backing TLS Secret.
.DESCRIPTION
    cert-manager detects the Secret is missing and reissues immediately, no
    cmctl dependency needed. Meant to be called right after a Root CA is
    freshly (re)generated on a mount (fresh-reinit / storage-mode switch, see
    33-openbao/Install.ps1): every Certificate previously issued against the
    OLD root becomes orphaned the moment any consumer re-reads the mount's
    *current* CA (e.g. 51-rancher's additionalTrustedCAs workaround), since
    the two no longer chain to the same root — this surfaced live as
    Rancher's OIDC calls to Authelia failing with "x509: certificate signed
    by unknown authority" after a reinit, even though nothing about
    Rancher's own config changed. A no-op on a genuine first install —
    nothing has been issued against $IssuerName yet.

    Shared here (rather than kept local to Install.ps1) so a one-off repair
    run (e.g. 33-openbao/Repair-OrphanedCertificates.ps1, for certs already
    orphaned by a rotation that happened before this mechanism existed) can
