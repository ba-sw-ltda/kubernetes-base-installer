...
# here (and duplicated into 11-ingress-nginx/Install.ps1) so it's the same
...
foreach ($ing in $existingIngresses['items']) {
    if ($ing['spec']['ingressClassName'] -ne "traefik") {
...
if ($updatedIngressCount -gt 0) {
    Write-GroupLine "✓ $updatedIngressCount Ingress resource(s) updated to ingressClassName=traefik" -ForegroundColor Green
} else {
